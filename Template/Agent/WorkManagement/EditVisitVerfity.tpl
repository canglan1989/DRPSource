...
<div class="form_edit">
    <div class="form_hd">
        <div class="form_hd_left"><div class="form_hd_right"><div class="form_hd_mid"><h2>修改质检记录</h2></div></div></div>
        <span class="declare">“<em class="require">*</em>”为必填信息</span>
    </div>
    <div class="form_bd">
        <form id="J_EditTelVerfity" name="J_EditTelVerfity">	
            <!--S form_block_bd--> 
            <div class="form_block_bd">
                    ...
                    <div class="form_block_left">
                        {foreach from=$VertifyListItemLeft item=data}
                            <div class="tf">
                                ...
                                <div class="inp">
                                    {if $data.checked == 2}
                                        <input type="checkbox" onclick="return false;" checked class="checkInp" name="item[{$data.item_id}]" value="{$data.item_name}" /><span style="color:red;">{$data.item_result}</span>
                                    {elseif $data.checked == 1}
                                        <input type="checkbox" checked  class="checkInp" name="item[{$data.item_id}]" value="{$data.item_name}" /><span style="color:green">{$data.item_result}</span>
                                    {else}
                                        <input type="checkbox" class="checkInp" name="item[{$data.item_id}]" value="{$data.item_name}" />{$data.item_result}
                                    {/if}
                                </div>
                            </div>
                        {/foreach}
                ...
                <div class="tf ">
                    <label>录音编号：</label>
                    <div class="inp">
                        <input name="record_no" value="{$VertifyInfo->strRecordNo}" />
                    </div>
                </div>
                <div class="tf ">
                    <label>质检结果：</label>
                    <div class="inp">
                        <select name="vertify_status" >
                            <option value="1" {if $NoteInfo->iIsVertifyed == 1 && $VertifyInfo->iVerfityStatus == "1"}selected{/if}>通过</option>
                            <option value="0" {if $NoteInfo->iIsVertifyed == 1 && $VertifyInfo->iVerfityStatus == "0"}selected{/if}>不通过</option>
                            <option value="-9" {if $NoteInfo->iIsVertifyed == 2}selected{/if} >不质检</option>
                        </select>
                    </div>
                </div>
                <div class="tf ">
                    <label>质检情况：</label>
                    <div class="inp">
                        <textarea name="vertify_remark" cols="50" rows="30">{$VertifyInfo->strVertifyRemark}</textarea>
                    </div>
                </div>
                    ...
                    <div id="J_ui_table" class="ui_table">
                        <table width="100%" cellspacing="0" cellpadding="0" border="0">
                            ...
                            <tbody class="ui_table_bd">
                                ...
                                <tr class="">
                                    <td title="{$data.vertify_id}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.vertify_id}</span></div></td>
                                    <td title="{$data.note_id}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.note_id}</span></div></td>
                                    <td title="{$data.record_no}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.record_no}</span></div></td>
                                    <td title="{$data.new_item_name}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.new_item_name}</span></div></td>
                                    <td title="{if $data.is_vertifyed == 2}不质检{elseif $data.verfity_status == 0}不通过{elseif $data.verfity_status == 1}通过{/if}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>
                                        {if $data.is_vertifyed == 2}不质检{elseif $data.verfity_status == 0}不通过{elseif $data.verfity_status == 1}通过{/if}
                                    </span></div></td>
                                    <td title="{$data.vertify_remark}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.vertify_remark}</span></div></td>
                                    <td title="{$data.create_user_name}"><div class="ui_table_tdcntr"><a href="javascript:void(0)" onclick="UserDetial({$data.create_uid});"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.create_user_name}</span></a></div></td>
                                    <td title="{$data.create_time}"><div class="ui_table_tdcntr"><span {if $VertifyInfo->iVertifyId == $data.vertify_id} style="color:green;"{/if}>{$data.create_time}</span></div></td>
                                </tr>   
                                {foreachelse}
...
<script type="text/javascript">
    {literal}
//验证代理商数据
new Reg.vf($('#J_EditTelVerfity'),{
        callback:function(data){
	 	if(!IM.IsSending(true)){return false;};
        $.ajax({
                type:'POST',
    {/literal}
                data:$('#J_EditTelVerfity').serialize()+"&id={$VertifyInfo->iVertifyId}",
                url:'{au d="WorkM" c="VisitVerify" a="EditVertify"}',
    {literal}
		dataType:'json',
