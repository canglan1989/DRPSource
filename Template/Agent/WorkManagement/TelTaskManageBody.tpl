{foreach from=$arrayData item=data key=index}
    <tr class="{sdrclass rIndex=$index}">
        ...
        <td title="{$data.sappoint_time|date_format:'%Y-%m-%d'}" ><div class="ui_table_tdcntr">{$data.sappoint_time|date_format:'%Y-%m-%d'}</div></td>
        <td title="{$data.title}" ><div class="ui_table_tdcntr">{$data.title|truncate:"54":"……"}</div></td>
            ...
            <div class="ui_table_tdcntr">
                <ul class="list_table_operation">
                    ...
                    {if $data.check_status == 1}
                        <li><a href="javascript:void(0);" m="TelTaskManage" v="32" ispurview="true" onclick="JumpPage('{au d="WorkM" c="TelWork" a="showAddTelNote"}&appid={$data.appoint_id}')">生成小记</a></li>	
                    {else}
                        {if $UserID == $data.create_id}
                    <li><a href="javascript:void(0);" m="TelTaskManage" v="4" ispurview="true" onclick="JumpPage('{au d="WorkM" c="TelWork" a="showAddTelInvite"}&appointid={$data.appoint_id}')">编辑</a></li>	
                    <li><a href="javascript:void(0);" m="TelTaskManage" v="8" ispurview="true" onclick="delTask({$data.appoint_id})">删除</a></li>
                    {/if}
                        {if $data.check_status == 0}
                    <li><a href="javascript:void(0);" m="TelTaskManage" v="16" ispurview="true" onclick="JumpPage('{au d="WorkM" c="TelWork" a="showCheckTelInvite"}&appid={$data.appoint_id}')">审核</a></li>        
                        {/if}
                    {/if}
                    {/if}
                </ul>
