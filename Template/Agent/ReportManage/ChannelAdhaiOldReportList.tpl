...
<div class="table_attention marginBottom10">
    <label>统计信息：</label>
    <span class="ui_link">累计新开：(<em>{$COUNT}</em>)</span>
        ...
        <div class="table_filter_main" id="J_table_filter_main">
            <div class="table_filter_main_row">  
                ...
                <div class="ui_title">统计月份：</div>
                    <div class="ui_text"><input type="text" onfocus="WdatePicker({literal}{onpicked:function(){($dp.$('create_timeE')).focus()},maxDate:'#F{$dp.$D(\'create_timeE\')}',dateFmt:'yyyy-MM'}{/literal})" name="create_timeS" id="create_timeS" class="inpCommon inpDate"> 
                     至 
                    <input type="text" onfocus="WdatePicker({literal}{minDate:'#F{$dp.$D(\'create_timeS\')}',dateFmt:'yyyy-MM'}{/literal})" name="create_timeE" id="create_timeE" class="inpCommon inpDate"></div>                  
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="0">
            <thead class="ui_table_hd">
                <tr>
                    ...
                    <th width="150" title="起始时间">
                        <div class="ui_table_thcntr ">
                            <div class="ui_table_thtext">起始时间</div>
                        </div>
                    </th>
                    <th width="150" title="结束时间">
                        <div class="ui_table_thcntr ">
                            <div class="ui_table_thtext">结束时间</div>
                        </div>
                    </th>
            <th width="150" title="本月累计新开">
            <div class="ui_table_thcntr" sort="sort_`new_count`.`count`">
...
{
    window.open("/?d=Agent&c=ReportManage&a=DownLoad_ChannelAdhaiOldReportBody" + pageList.param + "&sortField=" + pageList.sortField);
}
...
function jump2sign(username,begintime,endtime){
    var userrealname = encodeURIComponent(username);
    JumpPage("/?d=Agent&c=AgentMove&a=SignDetailIndex&username="+userrealname+"&begintime="+begintime+"&endtime="+endtime+"&pacttype=1");
}
    {/literal}
</script>
