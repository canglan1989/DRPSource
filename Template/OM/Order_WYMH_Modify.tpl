    ...
    <div class="form_block_bd">
        ...
        {if $id > 0 }
    		...
    		<div class="tf">
                ...
                <div class="inp">
                    <select id="cbDomainProvider" name="cbDomainProvider">
                    	<option {if $data.website_provider == ""} selected="selected" {/if} value="">请选择</option>
                    	<option {if $data.website_provider == "代理商提供"} selected="selected" {/if} value="代理商提供">代理商提供</option>
                    	<option {if $data.website_provider == "客户提供"} selected="selected" {/if} value="客户提供">客户提供</option>
                    </select>
                </div>
                <label style="width:70px">域名名称：</label>
                <div class="inp"><input  value="{$data.website_name}" type="text" valid="required url"  maxlength="50" name="tbxWebSite"/></div>            
                <div class="inp">
                	<a class="ui_button ui_link" control="add" href="javascript:;" style="margin:0 5px 0 0; float:left;"><div class="ui_icon ui_icon_add2">&nbsp;</div></a>
                    {if $index > 0}
                    <a class="ui_button ui_link" control="del" href="javascript:;" style="margin:0 5px 0 0; float:left;"><div class="ui_icon ui_icon_del2">&nbsp;</div></a>
                    {/if}
            	</div>
    			<span class="info">请输入域名名称</span>
    			<span class="ok">&nbsp;</span><span class="err">请输入域名名称</span>
        	</div>
            {/foreach}
        {else}
		<div class="tf">
            ...
            <div class="inp">
                <select id="cbDomainProvider" name="cbDomainProvider">
                    ...
                    <option value="代理商提供">代理商提供</option>
                    <option value="客户提供">客户提供</option>
                </select>
            </div>
            <label style="width:70px">域名名称：</label>
            <div class="inp"><input value="" type="text" valid="required url"  maxlength="50" id="tbxWebSite" name="tbxWebSite"/></div>            
            <div class="inp"><a class="ui_button ui_link" control="add" href="javascript:;" style="margin:0 5px 0 0; float:left;"><div class="ui_icon ui_icon_add2">&nbsp;</div></a></div>
			<span class="info">请输入域名名称</span>
			<span class="ok">&nbsp;</span><span class="err">请输入域名名称</span>
        </div>
        {/if}        
        ...
        <div class="tf">
        	<label>客服电话：</label>
            <div class="inp">
                <input name="tbxServiceTel" type="text" id="tbxServiceTel" style="width:150px;" size="30" maxlength="20" value="{$objOrder->strServiceTel}" />
            </div>
            <span class="info">请填写公司客服电话，以供账号开通时使用&nbsp;</span>
            <span class="ok">&nbsp;</span><span class="err">请填写公司客服电话，以供账号开通时使用&nbsp;</span>
        </div>
        </div>
...
(function(){
    ...
    function cb(formData){
	    ...
	    var cbDomainProviders = document.getElementsByName("cbDomainProvider");
       	var tbxWebSites = document.getElementsByName("tbxWebSite");
    	var domainProviders = "";
        var webSites = "";
        
    	for(var i = 0;i < cbDomainProviders.length;i++)
    	{
    		if(cbDomainProviders[i].value != "" && tbxWebSites[i].value != "")
            {
    			domainProviders += "," + cbDomainProviders[i].value;
                webSites += "," + tbxWebSites[i].value;
            }
    	}
    	
    	if(domainProviders.length > 1)
        {
    		domainProviders = domainProviders.substring(1, domainProviders.length); 
    		webSites = webSites.substring(1, webSites.length);            
        }       
		...
		MM.ajax({
			...
			data:$('#J_addOrder').serialize()+"&productNo="+productNo+"&id="+id+"&domainProviders="+domainProviders
            +"&webSites="+webSites,
	...
	function addHandler(event){
		var tplMain='<div class="tf">\
	                ...
	                <label style="width:70px">域名名称：</label>                 \
	                <div class="inp"><input type="text" valid="{1} url" name="tbxWebSite"></div>\
	                <div class="inp">\
					...
					<span class="info">请输入域名名称</span>\
	                <span class="ok">&nbsp;</span><span class="err">请正确输入域名名称</span>\
	            </div>';
	    var a=(MM.E(event).target).parentNode;
	    if(a.tagName=='A'){
	        var b=MM.A(a,'control');
	        if(b=='add'){
	            J_domainArea.append(_(tplMain,'',''));
	        }else if(b=='del'){
	...
	addFormEvent();
	$('#J_domainArea').unbind('click').bind('click',addHandler);
