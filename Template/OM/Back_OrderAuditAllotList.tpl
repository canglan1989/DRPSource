...
<div class="table_attention marginBottom10">
        <label>审核状态提醒：</label>
        <span class="ui_link">已分配：(<em>{$isAllolt}</em>)</span>
        <span class="ui_link">未分配：(<em>{$notAllolt}</em>)</span>
        <span class="ui_link">审核通过：(<em>{$isPass}</em>)</span>
        <span class="ui_link">审核未通过：(<em>{$notPass}</em>)</span>
        <span class="ui_link">未审核：(<em>{$auditting}</em>)</span>
</div>
<!--S table_filter-->
...
var _InDealWith = false;
 $(function(){
	...
	{/literal}
	pageList.strUrl = "{$OrderAuditAllotListBody}"; 
	{literal}
