...
<div class="form_bd">
    ...
    <div class="tf wy">
        ...
        <option value="{$data.agent_bank_id}" {if $objPostMoneyInfo->iAgentBankId == $data.agent_bank_id} selected="selected"{/if} >{$data.bank_name} {$data.account_name} {$data.account_no}</option>  
        {/foreach}
        </select>
        </div>
    ...
    <div class="tf wy">
        ...
        <div class="inp">
            ...
            <option value="{$data.ba_account_id}" {if $objPostMoneyInfo->iBankId == $data.ba_account_id} selected="selected"{/if}>{$data.ba_account_name} {$data.ba_account_no}</option>  
            {/foreach}
            </select>
        </div>
    ...
    <div class="tf">
        ...
        {if $objPostMoneyInfo->strRpFiles == ""}
            <div class="fileBtn">
	        	...
	        	<input name="permitJ_upload0" id="permitJ_upload0" type="hidden" value="" />
        ...
        {else}
            <div class="fileBtn">
	        	<input id="J_upload0" style="width:250px;" type="file" name="J_upload0"/>
	        	<input name="permitJ_upload0" id="permitJ_upload0" type="hidden" value="/?d=FM&c=PayMoney&a=ViewImage&id={$objPostMoneyInfo->iPostMoneyId}" />
    ...
    <div class="tf">
	    ...
	    <span class="c_info" style="color: #F00;">合计金额必需与底单扫描件中打款金额一致；一张底单扫描件只能提交一次打款。</span>
    </div>
    <div class="tf">
       <label>备注：</label>
       <div class="inp">
...
$(function(){
    ...
    new Reg.vf($('#J_Guarantee'),{
    	...
    	$.ajax({
    	    url:submitUrl,
    	    data:$('#J_Guarantee').serialize()+moneys+"&payTypeName="+encodeURIComponent(payTypeName)
    	    ...
    	    success:function(backData){
    	    	if(backData.indexOf("seccess") == 0)
                {
                    ...
                    id = id[1];
                    {/literal}
                    JumpPage("/?d=FM&c=PayMoney&a=PayMoneyDetail&id="+id,false);
                    {literal}
                }    
                else
                {
