...
{foreach from=$arrayUserRight item=data key=index}
{if $data.mgroup_name != ""} 
{if $index > 0 }
</tbody>
...
<tr class="ui_table_tr_sub{$data.model_id} {sdrclass rIndex=$index}">
  ...
  <td><div class="ui_table_tdcntr"> <a m="UserRightList" v="4" ispurview="true" href="javascript:;" onclick="CheckRight(this)" id="a_{$data.right_id}" >{if $data.is_check == 1}取消{else}分配{/if}</a> </div></td>
